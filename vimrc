" =============================================================================
"        << Part 1. 判断操作系统是Windows还是Linux，和判断是终端还是Gvim >>
" =============================================================================

" -----------------------------------------------------------------------------
"  < 1.1 判断操作系统是否是Windows还是Linux >
" -----------------------------------------------------------------------------
let g:iswindows = 0
let g:islinux = 0
...
" -----------------------------------------------------------------------------
"  < 2.2 Linux Gvim/Vim 默认配置 > 做了一点修改
" -----------------------------------------------------------------------------
if g:islinux
    set hlsearch        "高亮搜索
...
set fileencodings=ucs-bom,utf-8,gb18030,gbk,cp936,latin-1     "设置支持打开的文件的编码
...
" F7 一键保存并连接
nmap <F7> :call Link()<CR>
imap <F7> <ESC>:call Link()<CR>
...
func! Run()
    ...
    if expand("%:e") == "c" || expand("%:e") == "cpp" || expand("%:e") == "cxx"
        ...
        if executable(Exe) && getftime(Exe) >= getftime(Obj) && getftime(Obj) >= getftime(Sou)
            ...
            redraw!
            echohl WarningMsg | echo " running finished"
        endif
    elseif expand("%:e") == "java"
        let class = expand("%:p:r").s:Class_Extension
...
" <Leader>bh 水平分割窗口显示缓存列表，并在缓存列表窗口中打开选定文件
...
" -----------------------------------------------------------------------------
"  < 4.7 omnicppcomplete 插件配置 >
" -----------------------------------------------------------------------------
" 用于C/C++代码补全，这种补全主要针对命名空间、类、结构体、共同体等进行补全，详细
" 说明可以参考帮助或网络教程等
...
let Tlist_File_Fold_Auto_Close=1            "自动关闭其它文件的tags
let Tlist_WinWidth=30                       "设置窗口宽度
let Tlist_Use_Right_Window=1                "在右侧窗口中显示,0在左侧
let Tlist_Sort_Type = "name"				"以名称排序

" -----------------------------------------------------------------------------
"  < 4.12 txtbrowser 插件配置 >
" -----------------------------------------------------------------------------
" 用于文本文件生成标签与语法高亮（调用TagList插件生成标签，如果可以）
...
" =============================================================================
"                     << Part 7. windows 下解决 Quickfix 乱码问题 >>
" =============================================================================
